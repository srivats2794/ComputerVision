...
void ActiveContouring_Ext() {
	...
	for (k = 0;k < 30 ;k++) {
		...
		for (i = 0;i < size_Index + 1; i++) {
			...
			for (rc = -3; rc <= 3; rc++) {
				for (cc = -3; cc <= 3; cc++) {
					...
					E_ext[(3 + rc) * 7 + (3 + cc)] = varianceImage[(r + rc)*COLS + (c + cc)];
				}
			}
		...
		//Preserving old contour for redrawing image
		//for (m = 0;m<size_Index + 1;m++) {
		//	yContour_older[m] = yContour_old[m]; //r
		//	xContour_older[m] = xContour_old[m]; //c
		//}
...
void DrawDownsizedContour(){
	...
	unsigned char		*DisplayImage;
	unsigned char		*OriginalImage_wContour;
	OriginalImage_wContour = (unsigned char *)calloc(ROWS*COLS, 1);


	for (r = 0; r<ROWS; r++)
		for (c = 0; c<COLS; c++)
			OriginalImage_wContour[r*COLS + c] = OriginalImageGrey[r*COLS + c];
			...
			for (i1 = -2; i1 <= 2; i1++) {
				for (i2 = -2; i2 <= 2; i2++) {
					if (sqrt(i1*i1 + i2*i2) < 2.5) {
						//SetPixel(hDC, x_coord + i1, y_coord + i2, RGB(R, G, B));
						OriginalImage_wContour[((int)y_coord + i2)*COLS + ((int)x_coord + i1)] = 0;
					}
				}
			}
			x_coord += Xinc;
	...
	for (r = 0; r<ROWS; r++)
		for (c = 0; c<COLS; c++)
			DisplayImage[r*DISPLAY_COLS + c] = OriginalImage_wContour[r*COLS + c];
